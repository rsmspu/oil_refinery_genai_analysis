...
def main():
    ...
    # Title
    st.markdown('<h1 class="main-header">🛢️ Oil Rig Analysis Pipeline</h1>', unsafe_allow_html=True)
...
def run_demo_pipeline(months, n_logs, contamination, time_window):
    ...
    with st.spinner("🔄 Running complete analysis pipeline..."):
        ...
        status_text.text("📊 Generating synthetic oil rig data...")
        ...
        status_text.text("🔍 Detecting equipment anomalies...")
        ...
        status_text.text("📝 Correlating anomalies with operator logs...")
        ...
        status_text.text("🔮 Generating predictions and risk assessments...")
...
def display_demo_overview(months, n_logs, contamination):
    ...
    with col1:
        st.subheader("🎯 What This Demo Will Do")
    ...
    with col2:
        ...
        **Processing time:** ~10-15 seconds
        ...
        st.warning("⚡ Click 'Run Complete Analysis' to start the demo!")
...
def display_anomaly_analysis(results):
    ...
    st.subheader("📋 Recent High-Priority Anomalies")
...
def display_correlation_analysis(results):
    ...
    st.subheader("📝 Anomaly-Text Correlation Analysis")
    ...
    for idx, row in top_correlations.iterrows():
        with st.expander(f"🔗 {row['equipment'].replace('_', ' ').title()} - Score: {row['correlation_score']:.3f}"):
            col1, col2 = st.columns(2)
...
def display_predictions_and_risk(results):
    ...
    st.subheader("🔮 Predictive Analysis & Risk Assessment")
    ...
    st.subheader("🎯 Equipment-Specific Recommendations")
    ...
    for equipment, risk_data in sorted_equipment:
        risk_emoji = "🔴" if risk_data['risk_level'] == 'high' else "🟡" if risk_data['risk_level'] == 'medium' else "🟢"
...
<div style='text-align: center; color: #666; margin-top: 2rem;'>
    <p><b>🛢️ Oil Rig Analysis Pipeline - Standalone Demo</b></p>
    <p>Complete AI-powered solution for predictive maintenance and anomaly detection</p>
    <p><small>Built with Streamlit • Synthetic data for demonstration purposes</small></p>
</div>
""", unsafe_allow_html=True)
